...
urlfile_region = "https://raw.githubusercontent.com/Stat184-Fall2024/Sec1_FP_DanielLiu_JasonFicorilli/refs/heads/main/data/UNSD_codes.csv"
data_region <- read_csv(url(urlfile_region))
...
health_df %>% 
  select(Country, Country_Code, Year, Fertility_Rate, Urban_Population_Percent) %>%
  head() %>%
  knitr::kable() %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```

As we can see here, the data seems to be decently tidy - at first glance, the columns are well-named, and the numbers don't include any commas that would make computation difficult. The Country_Code column also is helpful for code brevity.

For this part of the research paper, we are only concerned with Country, Country_Code, Year, Water_Access_Percent, and Life_Expectancy. As such, we can get rid of all of the other columns for now and conduct our operations on a new dataframe, shown below.
...
data_water <- health_df %>% select(Country, Country_Code, Year, Water_Access_Percent, Life_Expectancy)
head(data_water) %>% 
  knitr::kable() %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
#| fig-cap: "Scatterplot of Water_Access_Percent Between 2012-2021"
#| column: page
#| echo: false
#| message: false
#| warning: false
...
ggplot(
  data = data_water,
  aes(x = Water_Access_Percent, y = Life_Expectancy)) +
  geom_point(color = "grey") +        # larger, semi-transparent points
  geom_smooth(method = "lm",                 # add trend line
             color = "black",
             se = TRUE,                      # include confidence interval
             alpha = 0.2) +
  theme_bw() +
  labs(
    title = "Relationship Between Water Access and Life Expectancy",
    x = "Access to Clean Water (%)",
    y = "Life Expectancy (Years)",                    # legend title
  ) +
  theme(
...
First, let's take a look at our secondary dataset, which includes some region information.
...
data_region %>% 
  select("Global Code", "Global Name", "Region Code", "Region Name", "Sub-region Code") %>%
  head() %>%
  knitr::kable() %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```

For the purposes of our data, we'll only be needing the following columns: `Region Name`, and `ISO-alpha3 Code`. Let's filter by just those two columns.
...
data_region_relevant <- data_region %>% 
  select("Region Name", "ISO-alpha3 Code") 

data_region_relevant %>%
  head() %>%
  knitr::kable() %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
num_rows_2 <- data_region_relevant %>% nrow() %>% toString()
paste("Rows:", num_rows_2)
```
...
data_region_merged <- left_join(
  x = data_water,
  y = data_region_relevant,
...
Now, we have our original dataset with all of our necessary information, as well as an extra column that includes each specific country's Region. We can use this extra column to help see if its eventually stratification, at all, affects our visualization, or at least, gives us some insight as to what specific Regions MoreLife Ltd. should focus on in their research. Let's begin.
...
region_summary <- data_region_merged %>%
  group_by(`Region Name`) %>%
  summarize(
    Mean = round(mean(Water_Access_Percent, na.rm = TRUE), 2),
...
Judging from this table, we can suggest to MoreLife Ltd. to focus on Africa, given that they have the lowest average water access percentage, and to potentially pay closer attention to regions such as Oceania, since this range indicates some areas being extremely "wealthy", and other areas being more neglected.
...
ggplot(
  data = data_region_merged,
  aes(x = Water_Access_Percent, y = Life_Expectancy, color = `Region Name`)) +
...
ggplot(
  data = data_region_merged,
  aes(x = Water_Access_Percent, y = Life_Expectancy, color = `Region Name`)) +
  geom_density_2d(aes(color = `Region Name`),
                  linewidth = 0.8,    
                  alpha = 0.6,        
                  bins = 15,          
                  h = c(12, 6)) +     
