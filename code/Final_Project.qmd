...
data_water <- health_df %>% select(Country, Country_Code, Year, Water_Access_Percent, Life_Expectancy) # Selecting only the columns that concern us
head(data_water) %>% 
  knitr::kable() %>% # Kable styling, yet again
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
First, let's take a look at our secondary dataset, which includes some region information below in Table 3.
...
data_region_relevant %>%
  head() %>% # Showing only the head for aesthetics
  knitr::kable() %>% # Kable styling, yet again
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
data_region_merged <- left_join( # Performing a left join to prioritize our Global Health dataset
  x = data_water,
...
data_region_merged %>%
  head() %>% # Viewing only the head for aesthetics
  knitr::kable() %>% # Kable styling
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
region_summary <- data_region_merged %>%
  group_by(`Region Name`) %>% # Specifically grouping my region name to compare the different regions
  summarize(
  ...
  mutate(Range = round(Max - Min, 2)) # Rounding all of the values to 2 decimal points for readability
...
ggplot(
  data = data_region_merged, # Data
  aes(x = Water_Access_Percent, y = Life_Expectancy, color = `Region Name`)) +
  geom_point(alpha = 0.5, size = 1.2) + # Making the dots less opaque, but still large enough discerned
