...
As we can see in Figure 1, the data seems to be decently tidy - at first glance, the columns are well-named, and the numbers don't include any commas that would make computation difficult. The Country_Code column also is helpful for code brevity.

For this part of the research paper, we are only concerned with Country, Country_Code, Year, Water_Access_Percent, and Life_Expectancy. As such, we can get rid of all of the other columns for now and conduct our operations on a new dataframe, shown below, in Figure 2.
...
First, let's take a look at our secondary dataset, which includes some region information below in Figure 4.
...
For the purposes of our data, we'll only be needing the following columns: `Region Name`, and `ISO-alpha3 Code`. Let's filter by just those two columns (visible in Figure 5, below).
...
#| fig-cap: "Scatterplot of Water_Access_Percent Between 2012-2021, Colored By Region"
#| column: page
#| echo: false
#| message: false
#| warning: false
...
#| fig-cap: "Contour Plot of Water_Access_Percent Between 2012-2021, Colored By Region"
#| column: page
#| echo: false
#| message: false
#| warning: false
